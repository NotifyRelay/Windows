...
public partial class OverlayRenderService
{
    ...
    private static readonly uint[] TopmostMonitorEvents = { EVENT_OBJECT_FOCUS, EVENT_SYSTEM_FOREGROUND };
    private readonly List<IntPtr> _topmostHooks = [];
    private WinEventDelegate? _winEventProcDelegate;

    /// <summary>
    /// 注册置顶监控 WinEvent 钩子（渲染线程调用，回调也在渲染线程执行）。
    /// 每次调用先清理残留钩子再重建，保证渲染线程重启后监控仍然有效。
    /// </summary>
    private void InstallTopmostMonitor()
    {
        UninstallTopmostMonitor();
        _winEventProcDelegate = OnWinEvent;
        foreach (var evt in TopmostMonitorEvents)
        {
            var hook = SetWinEventHook(evt, evt, IntPtr.Zero, _winEventProcDelegate, 0, 0,
                WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS);
            if (hook != IntPtr.Zero)
                _topmostHooks.Add(hook);
        }
        if (_topmostHooks.Count == 0)
            _logger.LogWarning("注册置顶监控 WinEvent 钩子失败，仅依赖渲染循环周期巡检");
    }

    private void UninstallTopmostMonitor()
    {
        foreach (var hook in _topmostHooks)
            UnhookWinEvent(hook);
        _topmostHooks.Clear();
    ...
    /// <summary>
    /// WinEvent 回调：前台/焦点变化时检查覆盖层置顶是否被重置。
    /// 仅在 WS_EX_TOPMOST 标志丢失时恢复，不做无条件层级抢占
    /// （避免切换窗口时覆盖层抢占层级导致系统窗口无法操作）。
    /// </summary>
    ...
    private void EnsureOverlaysTopmost()
    {
        ...
        {
            if (!o.Visible || o.Hwnd == IntPtr.Zero || !IsWindow(o.Hwnd)) continue;
    ...
    /// </summary>
    private void ReassertOverlaysTopmost()
    {
        foreach (var o in _overlays)
        {
            if (!o.Visible || o.Hwnd == IntPtr.Zero || !IsWindow(o.Hwnd)) continue;
